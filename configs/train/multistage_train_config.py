"""
多阶段有监督训练配置文件
支持使用原始数据集或清晰度平均化后的数据集进行训练

数据集配置说明：
1. 当前配置使用清晰度平均化后的数据集（推荐）
   - 训练和验证JSON: "master_sharpness_averaged_dataset.json"
   - 图像路径: datasets/dataset*/sharpness_averaged/
   - 掩码路径: datasets/dataset*/masks_3class/

2. 如需使用原始数据集，请注释掉清晰度平均化配置，取消注释原始数据集配置

3. 确保在运行训练之前已经运行了图像清晰度处理器
"""
...
STAGES = [
    {
        "name": "stage1",
        # 使用清晰度平均化后的数据集（推荐）
        "train_json": "master_sharpness_averaged_dataset.json",
        "val_json": "master_sharpness_averaged_dataset.json",
...
log_dir = cfg_base.BASE_LOG_DIR / LOGGER / f"multistage_train_{ENCODER_NAME}_{DECODER_NAME}"
log_config = {
    "logger": LOGGER,
    "project": "SRPU-Model",
    "log_dir": str(log_dir)
