...
class SemSegmentationDataset(Dataset):
    ...
    def __init__(self,
        ...
        # 构建完整的JSON文件路径
        # 检查 json_file_identifier 是否已经是完整路径
        if isinstance(json_file_identifier, str) and json_file_identifier.endswith('.json'):
            # 如果是相对路径（包含目录分隔符），直接使用
            if '/' in json_file_identifier or '\\' in json_file_identifier:
                self.json_path = self.project_root / json_file_identifier
            else:
                # 否则假设在 json 目录下
                self.json_path = self.project_root / cfg_json.JSON_OUTPUT_DIR_NAME / json_file_identifier
        else:
            # 默认在 json 目录下
            self.json_path = self.project_root / cfg_json.JSON_OUTPUT_DIR_NAME / f"{json_file_identifier}.json"
